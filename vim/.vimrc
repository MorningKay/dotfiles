...
" * FZF
if executable("brew") | execute "set rtp+=" . trim(system("brew --prefix")) . "/opt/fzf" | endif
...
" clean Windows/WSL ^M
nnoremap <silent> <leader>cr :keeppatterns %s/\r\+$//e<CR>
xnoremap <silent> <leader>cr :keeppatterns s/\r\+$//e<CR>
...
" ========= Terminal & Session (minimal) =========
" <Space> t: turn current window into a terminal
nnoremap <silent> <leader>t :terminal<CR>

" <Space> Shift+T: open a terminal in a new tab
nnoremap <silent> <leader>T :tabnew \| terminal<CR>

" --- Session save/restore (so you can quit back to your shell and resume later) ---
let g:my_session_dir = expand('~/.vim/sessions')
if !isdirectory(g:my_session_dir) | call mkdir(g:my_session_dir, 'p') | endif

function! s:SaveSession(name) abort
  execute 'mksession!' fnameescape(g:my_session_dir.'/'.a:name.'.vim')
endfunction

function! s:LoadSession(name) abort
  execute 'source' fnameescape(g:my_session_dir.'/'.a:name.'.vim')
endfunction

" <Space> q s: save session and quit (return to the shell you launched Vim from)
nnoremap <silent> <leader>qs :call <SID>SaveSession('last')<CR>:qall<CR>

" <Space> q r: restore last session
nnoremap <silent> <leader>qr :call <SID>LoadSession('last')<CR>

" --- WhichKey labels (only these four entries) ---
let g:which_key_map = get(g:, 'which_key_map', {})

let g:which_key_map.t = get(g:which_key_map, 't', {'name': '+terminal'})
let g:which_key_map.t.t = 'Current window: :terminal'
let g:which_key_map.t.T = 'New tab: :terminal'

let g:which_key_map.q = get(g:which_key_map, 'q', {'name': '+quit/session'})
let g:which_key_map.q.s = 'Save session & quit'
let g:which_key_map.q.r = 'Restore last session'

" --- Remove ^M at EOL ---
let g:which_key_map = get(g:, 'which_key_map', {})
let g:which_key_map.c = get(g:which_key_map, 'c', {'name': '+clean'})
let g:which_key_map.c.r = 'Strip ^M (CR) at EOL'
